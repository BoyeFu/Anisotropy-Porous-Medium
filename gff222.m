...
x2=1;%the high limit of the integration, becuase the integration region is (0,infinity) in equation (A13),we use the value x2 represent infinity
x1=0.0;
NMAX=10;
...
for i=1:NMAX;
    FFF(i)=gff22(u(i)).*u(i).*besselj(0,z.*u(i)).*w(i);
end
sumff=sum(FFF);
%%
g=sumff;
